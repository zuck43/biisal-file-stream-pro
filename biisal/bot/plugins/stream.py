...
msg_text ="""<b>‣ ʏᴏᴜʀ ʟɪɴᴋ ɢᴇɴᴇʀᴀᴛᴇᴅ ! Nasir😎
...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        stream_link = f"{Var.URL}watch/{str(log_msg.id)}/{get_hash(log_msg)}/{quote_plus(get_name(log_msg))}"
        online_link = f"{Var.URL}{str(log_msg.id)}/{get_hash(log_msg)}/{quote_plus(get_name(log_msg))}"
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        stream_link = f"{Var.URL}watch/{str(log_msg.id)}/?hash={get_hash(log_msg)}{quote_plus(get_name(log_msg))}"
        online_link = f"{Var.URL}{str(log_msg.id)}/?hash={get_hash(log_msg)}{quote_plus(get_name(log_msg))}"
